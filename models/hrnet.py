...
class HighResolutionNet(nn.Module):
    def __init__(self, num_classes):
        ...
        self.last_layer = self._make_last_layer(last_inp_channels, num_classes)
        self.pseudo_last_layer = self._make_last_layer(last_inp_channels, num_classes)
    ...
    def get_pseudo_predicts(self, x, size=None):
        x = self.pseudo_last_layer(x)
        if size is not None:
            x = F.interpolate(x, size=size, mode='bilinear')
    ...
    def forward(self, x, return_pseudo_pred=False):
        ...
        pred = self.get_predicts(x, size=[img_h, img_w])

        if return_pseudo_pred:
            pseudo_pred = self.get_pseudo_predicts(x, size=[img_h, img_w])
    ...
    def get_1x_lr_params_NOscale(self):
        ...
        b.append(self.stage2)
        b.append(self.stage3)
        b.append(self.stage4)
...
def HRNet(num_classes):
    ...
    model.init_weights('./checkpoints/hrnetv2_w48_imagenet_pretrained.pth')
