...
class mit_b0(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            ...
            self.load_state_dict(torch.load("checkpoints/segformer_b0_backbone_weights.pth"), strict=False)
...
class mit_b1(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            self.load_state_dict(torch.load("checkpoints/segformer_b1_backbone_weights.pth"), strict=False)
...
class mit_b2(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            self.load_state_dict(torch.load("checkpoints/segformer_b2_backbone_weights.pth"), strict=False)
...
class mit_b3(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            self.load_state_dict(torch.load("checkpoints/segformer_b3_backbone_weights.pth"), strict=False)
...
class mit_b4(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            self.load_state_dict(torch.load("checkpoints/segformer_b4_backbone_weights.pth"), strict=False)
...
class mit_b5(MixVisionTransformer):
    def __init__(self, pretrained = False):
        ...
        if pretrained:
            print("Load backbone weights")
            self.load_state_dict(torch.load("checkpoints/segformer_b5_backbone_weights.pth"), strict=False)
...
class SegFormerHead(nn.Module):
    ...
    def __init__(self, num_classes=20, in_channels=[32, 64, 160, 256], embedding_dim=768, dropout_ratio=0.1):
        ...
        self.linear_pred = self._make_last_layer(embedding_dim*4, num_classes)
        self.pseudo_linear_pred = self._make_last_layer(embedding_dim*4, num_classes)
...
class SegFormer(nn.Module):
    def __init__(self, num_classes=2, phi='b0', pretrained=True):
        ...
        self.decoder = SegFormerHead(num_classes, self.in_channels, self.embedding_dim)

    def get_features(self, inputs):
        feat = self.backbone.forward(inputs)
        feat = self.decoder.fuse_features(feat)
        return feat

    def get_predicts(self, inputs, size=None):
        return self.decoder.get_predicts(inputs, size)

    def get_pseudo_predicts(self, inputs, size=None):
        return self.decoder.get_pseudo_predicts(inputs, size)

    def forward(self, image, size=None, return_pseudo_pred=False):
        H, W = image.size(2), image.size(3)
        feat = self.get_features(image)
        pred = self.get_predicts(feat)
        pred = F.interpolate(pred, size=(H, W), mode='bilinear', align_corners=False)
        if return_pseudo_pred:
            pseudo_pred = self.get_pseudo_predicts(feat)
            pseudo_pred = F.interpolate(pseudo_pred, size=(H, W), mode='bilinear', align_corners=False)
